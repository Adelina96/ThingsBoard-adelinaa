...
import org.thingsboard.server.dao.camera.CameraService;
...
public abstract class BaseController {
        ...
        try {
            ...
            switch (entityId.getEntityType()) {
                ...
                case DEVICE:
                    ...
                    return;
                case TESTING:
                    checkCameraId(new CameraId(entityId.getId()), operation);
                    return;
                case DEVICE_PROFILE:
    ...
    Camera checkCameraId(CameraId cameraId, Operation operation) throws ThingsboardException {
        try {
            validateId(cameraId, "Incorrect deviceId " + cameraId);
            Camera camera = cameraService.findCameraById(getCurrentUser().getTenantId(), cameraId);
            checkNotNull(camera);
            accessControlService.checkPermission(getCurrentUser(), Resource.DEVICE, operation, cameraId, camera);
            return camera;
        } catch (Exception e) {
            throw handleException(e, false);
        }
    ...
    CameraInfo checkCameraInfoId(CameraId deviceId, Operation operation) throws ThingsboardException {
        try {
            validateId(deviceId, "Incorrect deviceId " + deviceId);
            CameraInfo device = cameraService.findCameraInfoById(getCurrentUser().getTenantId(), deviceId);
            checkNotNull(device);
